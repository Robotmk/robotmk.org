...
> **Info:** RCC bestimmt sein Basisverzeichnis (`ROBOCORP_HOME`) abhängig vom ausführenden Benutzer. Für normale Benutzer ist das immer im Home-Dir.  
> Läuft RCC dagegen **als root**, ist der Default unter Linux `/opt/robocorp`.

Und genau hier wurde unter SLES ein seltsames Verhalten beobachtet: Wenn dieses Verzeichnis noch **nicht existierte** und RCC es als root anlegte, modifizierte es gleich die Rechte des *Elternverzeichnisses* `/opt`.
...
Die Reaktion im Mai war die naheliegende: Wenn eine Kombination Schaden anrichten kann, lassen wir sie lieber nicht erst zu.  
...
Hinzu kommt, dass die Option nur in der Regel *"Robotmk Scheduler (Linux)"* auftaucht. Wer die Bakery gar nicht benutzt und Robotmk z.b. per Ansible deployt, kommt an der Stelle gar nicht vorbei.
...
Das eigentliche Problem war ja nie "*root plus RCC*" als solches, sondern ein sehr spezifischer Nebeneffekt beim **erstmaligen Anlegen** eines Verzeichnisses unterhalb von `/opt` **unter SLES16**.  
...
Vielleicht wirkt die ganze Geschichte auf den ersten Blick ein bisschen Hü-Hott.
...
Es traut dem Admin natürlich etwas zu. Doku lesen (zumindest die Inline-Hilfe) war schon immer eine gute Idee. 😉
