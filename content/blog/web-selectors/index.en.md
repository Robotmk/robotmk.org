...
Of course, even the **Accessible Name** is no panacea. 
I’d like to (almost) conclude this article with a clear recommendation: use selector types in this order (sorted from most stable to most fragile):
**1) Automation IDs**: `id`, `data-testid` & Co.
  
Advantages: 
- ...unbeatable! Specifically designed for **automation**
...
The only catch: it must be set by the developers.  
Without their cooperation, the ID simply does not exist. 
**Example:**
```html
...
**2) Role + Accessible Name**: `role=...[name=‘...’]`

- the **sweet spot** 😊
- semantic rather than structural; it **reflects the user’s intention**
- also works **without** the developer’s help – provided the page is reasonably accessible
...
- **language-dependent**
- ⚠️ **Caution**: Playwright’s text strategy is, by default, **partial string matching and case-insensitive**. `text=Save` therefore also matches ‘**Save** and close’ – a classic pitfall.  
Using quotation marks forces **exact matching**:

```robotframework
Click    text=Save      # Substring, case-insensitive – be careful!
Click    text="Save"    # exact
```
...
**4) CSS/XPath on other attributes**
...
Only use them if there really is no other stable anchor available. 

```robotframework
Click    input[name=“email”]      # CSS: input with the attribute name="email"
